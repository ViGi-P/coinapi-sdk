...
static bool v1ExchangerateAssetIdBaseAssetIdQuoteHistoryGetProcessor(MemoryStruct_s p_chunk, long code, char* errormsg, void* userData,
	void(* voidHandler)())
{
	void(* handler)(std::list<V1.ExchangeRatesTimeseriesItem>, Error, void* )
	= reinterpret_cast<void(*)(std::list<V1.ExchangeRatesTimeseriesItem>, Error, void* )> (voidHandler);
	
	JsonNode* pJson;
	char * data = p_chunk.memory;

	std::list<V1.ExchangeRatesTimeseriesItem> out;
	...
	if (code >= 200 && code < 300) {
		...
		for(guint i = 0; i < length; i++){
			...
			V1.ExchangeRatesTimeseriesItem singlemodel;
	...
	, void* userData, bool isAsync)
{
	...
	string url("/v1/exchangerate/{asset_id_base}/{asset_id_quote}/history");
	int pos;
	...
	if(!isAsync){
		NetClient::easycurl(ExchangeRatesManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg);
		bool retval = v1ExchangerateAssetIdBaseAssetIdQuoteHistoryGetProcessor(*p_chunk, code, errormsg, userData,reinterpret_cast<void(*)()>(handler));
		...
		requestInfo = new(nothrow) RequestInfo (ExchangeRatesManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), v1ExchangerateAssetIdBaseAssetIdQuoteHistoryGetProcessor);;
		if(requestInfo == NULL)
			return false;
	...
	, void* userData)
{
	...
	assetIdBase, filterAssetId, invert, time, 
	handler, userData, true);
}
...
static bool v1ExchangerateHistoryPeriodsGetProcessor(MemoryStruct_s p_chunk, long code, char* errormsg, void* userData,
	void(* voidHandler)())
{
...
static bool v1ExchangerateHistoryPeriodsGetHelper(char * accessToken,
	...
	, void* userData, bool isAsync)
{
	...
	string url("/v1/exchangerate/history/periods");
	int pos;
	...
	if(!isAsync){
		NetClient::easycurl(ExchangeRatesManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg);
		bool retval = v1ExchangerateHistoryPeriodsGetProcessor(*p_chunk, code, errormsg, userData,reinterpret_cast<void(*)()>(handler));
		...
		requestInfo = new(nothrow) RequestInfo (ExchangeRatesManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), v1ExchangerateHistoryPeriodsGetProcessor);;
		if(requestInfo == NULL)
			return false;
...
bool ExchangeRatesManager::v1ExchangerateHistoryPeriodsGetAsync(char * accessToken,
	
	void(* handler)(std::list<V1.TimeseriesPeriod>, Error, void* )
	, void* userData)
{
	return v1ExchangerateHistoryPeriodsGetHelper(accessToken,
	
	handler, userData, true);
}

bool ExchangeRatesManager::v1ExchangerateHistoryPeriodsGetSync(char * accessToken,
