import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for OHLCVTimeSeriesExchangeTimeseriesItem
void main() {
  final instance = OHLCVTimeSeriesExchangeTimeseriesItemBuilder();
  // TODO add properties to the builder and call build()

  group(OHLCVTimeSeriesExchangeTimeseriesItem, () {
    // Gets or sets the start time of the time period.
    // DateTime timePeriodStart
    test('to test the property `timePeriodStart`', () async {
      // TODO
    });

    // Gets or sets the end time of the time period.
    // DateTime timePeriodEnd
    test('to test the property `timePeriodEnd`', () async {
      // TODO
    });

    // Gets or sets the time when the price opened.
    // DateTime timeOpen
    test('to test the property `timeOpen`', () async {
      // TODO
    });

    // Gets or sets the time when the price closed.
    // DateTime timeClose
    test('to test the property `timeClose`', () async {
      // TODO
    });

    // Gets or sets the opening price.
    // double priceOpen
    test('to test the property `priceOpen`', () async {
      // TODO
    });

    // Gets or sets the highest price during the time period.
    // double priceHigh
    test('to test the property `priceHigh`', () async {
      // TODO
    });

    // Gets or sets the lowest price during the time period.
    // double priceLow
    test('to test the property `priceLow`', () async {
      // TODO
    });

    // Gets or sets the closing price.
    // double priceClose
    test('to test the property `priceClose`', () async {
      // TODO
    });

    // Gets or sets the total volume traded during the time period.
    // double volumeTraded
    test('to test the property `volumeTraded`', () async {
      // TODO
    });

    // Gets or sets the number of trades executed during the time period.
    // int tradesCount
    test('to test the property `tradesCount`', () async {
      // TODO
    });

    // String symbolIdExchange
    test('to test the property `symbolIdExchange`', () async {
      // TODO
    });

  });
}
