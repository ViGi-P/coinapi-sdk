#' Create a new ModelsTradingStatusModel
#'
#' @description
#' Represents the response DTO for trading status information
#'
#' @docType class
#' @title ModelsTradingStatusModel
#' @description ModelsTradingStatusModel Class
...
#' @export
ModelsTradingStatusModel <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ModelsTradingStatusModel class.
    #'
    ...
    #'
    #' @return ModelsTradingStatusModel as a base R list.
    #' @examples
    #' # convert array of ModelsTradingStatusModel (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ModelsTradingStatusModel to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ModelsTradingStatusModelObject <- list()
      if (!is.null(self$`symbol`)) {
        ModelsTradingStatusModelObject[["symbol"]] <-
          self$`symbol`
      }
      if (!is.null(self$`timestamp_nanos`)) {
        ModelsTradingStatusModelObject[["timestamp_nanos"]] <-
          self$`timestamp_nanos`
      }
      if (!is.null(self$`timestamp`)) {
        ModelsTradingStatusModelObject[["timestamp"]] <-
          self$`timestamp`
      }
      if (!is.null(self$`is_trading_live`)) {
        ModelsTradingStatusModelObject[["is_trading_live"]] <-
          self$`is_trading_live`
      }
      if (!is.null(self$`is_trading_halted`)) {
        ModelsTradingStatusModelObject[["is_trading_halted"]] <-
          self$`is_trading_halted`
      }
      if (!is.null(self$`is_trading_in_order_acceptance_period`)) {
        ModelsTradingStatusModelObject[["is_trading_in_order_acceptance_period"]] <-
          self$`is_trading_in_order_acceptance_period`
      }
      if (!is.null(self$`is_trading_paused`)) {
        ModelsTradingStatusModelObject[["is_trading_paused"]] <-
          self$`is_trading_paused`
      }
      if (!is.null(self$`is_reason_halt_news_pending`)) {
        ModelsTradingStatusModelObject[["is_reason_halt_news_pending"]] <-
          self$`is_reason_halt_news_pending`
      }
      if (!is.null(self$`is_reason_ipo_not_yet_trading`)) {
        ModelsTradingStatusModelObject[["is_reason_ipo_not_yet_trading"]] <-
          self$`is_reason_ipo_not_yet_trading`
      }
      if (!is.null(self$`is_reason_ipo_deferred`)) {
        ModelsTradingStatusModelObject[["is_reason_ipo_deferred"]] <-
          self$`is_reason_ipo_deferred`
      }
      if (!is.null(self$`is_reason_halt_news_dissemination`)) {
        ModelsTradingStatusModelObject[["is_reason_halt_news_dissemination"]] <-
          self$`is_reason_halt_news_dissemination`
      }
      if (!is.null(self$`is_reason_ipo_order_acceptance_period`)) {
        ModelsTradingStatusModelObject[["is_reason_ipo_order_acceptance_period"]] <-
          self$`is_reason_ipo_order_acceptance_period`
      }
      if (!is.null(self$`is_reason_ipo_pre_launch_period`)) {
        ModelsTradingStatusModelObject[["is_reason_ipo_pre_launch_period"]] <-
          self$`is_reason_ipo_pre_launch_period`
      }
      if (!is.null(self$`is_reason_market_wide_circuit_breaker_level1`)) {
        ModelsTradingStatusModelObject[["is_reason_market_wide_circuit_breaker_level1"]] <-
          self$`is_reason_market_wide_circuit_breaker_level1`
      }
      if (!is.null(self$`is_reason_market_wide_circuit_breaker_level2`)) {
        ModelsTradingStatusModelObject[["is_reason_market_wide_circuit_breaker_level2"]] <-
          self$`is_reason_market_wide_circuit_breaker_level2`
      }
      if (!is.null(self$`is_reason_market_wide_circuit_breaker_level3`)) {
        ModelsTradingStatusModelObject[["is_reason_market_wide_circuit_breaker_level3"]] <-
          self$`is_reason_market_wide_circuit_breaker_level3`
      }
      if (!is.null(self$`is_reason_not_applicable`)) {
        ModelsTradingStatusModelObject[["is_reason_not_applicable"]] <-
          self$`is_reason_not_applicable`
      }
      if (!is.null(self$`is_reason_not_available`)) {
        ModelsTradingStatusModelObject[["is_reason_not_available"]] <-
          self$`is_reason_not_available`
      }
      return(ModelsTradingStatusModelObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ModelsTradingStatusModel
    #'
    #' @param input_json the JSON input
    #' @return the instance of ModelsTradingStatusModel
    ...
    #' @return ModelsTradingStatusModel in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ModelsTradingStatusModel and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ModelsTradingStatusModel
    toString = function() {
      self$toJSONString()
    },
...
# ModelsTradingStatusModel$unlock()
#
## Below is an example to define the print function
# ModelsTradingStatusModel$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ModelsTradingStatusModel$lock()
