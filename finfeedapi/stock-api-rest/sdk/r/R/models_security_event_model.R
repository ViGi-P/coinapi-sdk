#' Create a new ModelsSecurityEventModel
#'
#' @description
#' Represents the response DTO for security event information
#'
#' @docType class
#' @title ModelsSecurityEventModel
#' @description ModelsSecurityEventModel Class
...
#' @export
ModelsSecurityEventModel <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ModelsSecurityEventModel class.
    #'
    ...
    #'
    #' @return ModelsSecurityEventModel as a base R list.
    #' @examples
    #' # convert array of ModelsSecurityEventModel (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ModelsSecurityEventModel to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ModelsSecurityEventModelObject <- list()
      if (!is.null(self$`symbol`)) {
        ModelsSecurityEventModelObject[["symbol"]] <-
          self$`symbol`
      }
      if (!is.null(self$`timestamp_nanos`)) {
        ModelsSecurityEventModelObject[["timestamp_nanos"]] <-
          self$`timestamp_nanos`
      }
      if (!is.null(self$`timestamp`)) {
        ModelsSecurityEventModelObject[["timestamp"]] <-
          self$`timestamp`
      }
      if (!is.null(self$`security_event`)) {
        ModelsSecurityEventModelObject[["security_event"]] <-
          self$`security_event`
      }
      if (!is.null(self$`security_event_code`)) {
        ModelsSecurityEventModelObject[["security_event_code"]] <-
          self$`security_event_code`
      }
      if (!is.null(self$`security_event_text`)) {
        ModelsSecurityEventModelObject[["security_event_text"]] <-
          self$`security_event_text`
      }
      if (!is.null(self$`is_opening_process_complete`)) {
        ModelsSecurityEventModelObject[["is_opening_process_complete"]] <-
          self$`is_opening_process_complete`
      }
      if (!is.null(self$`is_closing_process_complete`)) {
        ModelsSecurityEventModelObject[["is_closing_process_complete"]] <-
          self$`is_closing_process_complete`
      }
      return(ModelsSecurityEventModelObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ModelsSecurityEventModel
    #'
    #' @param input_json the JSON input
    #' @return the instance of ModelsSecurityEventModel
    ...
    #' @return ModelsSecurityEventModel in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ModelsSecurityEventModel and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ModelsSecurityEventModel
    toString = function() {
      self$toJSONString()
    },
...
# ModelsSecurityEventModel$unlock()
#
## Below is an example to define the print function
# ModelsSecurityEventModel$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ModelsSecurityEventModel$lock()
