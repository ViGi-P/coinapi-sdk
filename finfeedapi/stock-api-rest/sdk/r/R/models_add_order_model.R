#' Create a new ModelsAddOrderModel
#'
#' @description
#' Represents the response DTO for add order information
#'
#' @docType class
#' @title ModelsAddOrderModel
#' @description ModelsAddOrderModel Class
...
#' @export
ModelsAddOrderModel <- R6::R6Class(
  "ModelsAddOrderModel",
  public = list(
    `symbol` = NULL,
    ...
    #' @description
    #' Initialize a new ModelsAddOrderModel class.
    #'
    ...
    #'
    #' @return ModelsAddOrderModel as a base R list.
    #' @examples
    #' # convert array of ModelsAddOrderModel (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ModelsAddOrderModel to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ModelsAddOrderModelObject <- list()
      if (!is.null(self$`symbol`)) {
        ModelsAddOrderModelObject[["symbol"]] <-
          self$`symbol`
      }
      if (!is.null(self$`timestamp_nanos`)) {
        ModelsAddOrderModelObject[["timestamp_nanos"]] <-
          self$`timestamp_nanos`
      }
      if (!is.null(self$`timestamp`)) {
        ModelsAddOrderModelObject[["timestamp"]] <-
          self$`timestamp`
      }
      if (!is.null(self$`is_side_buy`)) {
        ModelsAddOrderModelObject[["is_side_buy"]] <-
          self$`is_side_buy`
      }
      if (!is.null(self$`size`)) {
        ModelsAddOrderModelObject[["size"]] <-
          self$`size`
      }
      if (!is.null(self$`price`)) {
        ModelsAddOrderModelObject[["price"]] <-
          self$`price`
      }
      if (!is.null(self$`order_id`)) {
        ModelsAddOrderModelObject[["order_id"]] <-
          self$`order_id`
      }
      return(ModelsAddOrderModelObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ModelsAddOrderModel
    #'
    #' @param input_json the JSON input
    #' @return the instance of ModelsAddOrderModel
    ...
    #' @return ModelsAddOrderModel in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ModelsAddOrderModel and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ModelsAddOrderModel
    toString = function() {
      self$toJSONString()
    },
...
# ModelsAddOrderModel$unlock()
#
## Below is an example to define the print function
# ModelsAddOrderModel$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ModelsAddOrderModel$lock()
