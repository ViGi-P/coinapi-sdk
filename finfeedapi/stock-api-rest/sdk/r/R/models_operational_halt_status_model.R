#' Create a new ModelsOperationalHaltStatusModel
#'
#' @description
#' Represents the response DTO for operational halt status information
#'
#' @docType class
#' @title ModelsOperationalHaltStatusModel
#' @description ModelsOperationalHaltStatusModel Class
...
#' @export
ModelsOperationalHaltStatusModel <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ModelsOperationalHaltStatusModel class.
    #'
    ...
    #'
    #' @return ModelsOperationalHaltStatusModel as a base R list.
    #' @examples
    #' # convert array of ModelsOperationalHaltStatusModel (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ModelsOperationalHaltStatusModel to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      ModelsOperationalHaltStatusModelObject <- list()
      if (!is.null(self$`symbol`)) {
        ModelsOperationalHaltStatusModelObject[["symbol"]] <-
          self$`symbol`
      }
      if (!is.null(self$`timestamp_nanos`)) {
        ModelsOperationalHaltStatusModelObject[["timestamp_nanos"]] <-
          self$`timestamp_nanos`
      }
      if (!is.null(self$`timestamp`)) {
        ModelsOperationalHaltStatusModelObject[["timestamp"]] <-
          self$`timestamp`
      }
      if (!is.null(self$`operational_halt_status`)) {
        ModelsOperationalHaltStatusModelObject[["operational_halt_status"]] <-
          self$`operational_halt_status`
      }
      if (!is.null(self$`operational_halt_status_code`)) {
        ModelsOperationalHaltStatusModelObject[["operational_halt_status_code"]] <-
          self$`operational_halt_status_code`
      }
      if (!is.null(self$`operational_halt_status_text`)) {
        ModelsOperationalHaltStatusModelObject[["operational_halt_status_text"]] <-
          self$`operational_halt_status_text`
      }
      if (!is.null(self$`is_operationally_halted`)) {
        ModelsOperationalHaltStatusModelObject[["is_operationally_halted"]] <-
          self$`is_operationally_halted`
      }
      if (!is.null(self$`is_not_operationally_halted`)) {
        ModelsOperationalHaltStatusModelObject[["is_not_operationally_halted"]] <-
          self$`is_not_operationally_halted`
      }
      return(ModelsOperationalHaltStatusModelObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of ModelsOperationalHaltStatusModel
    #'
    #' @param input_json the JSON input
    #' @return the instance of ModelsOperationalHaltStatusModel
    ...
    #' @return ModelsOperationalHaltStatusModel in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ModelsOperationalHaltStatusModel and throw an exception if invalid
    #'
    ...
    #'
    #' @return String representation of ModelsOperationalHaltStatusModel
    toString = function() {
      self$toJSONString()
    },
...
# ModelsOperationalHaltStatusModel$unlock()
#
## Below is an example to define the print function
# ModelsOperationalHaltStatusModel$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# ModelsOperationalHaltStatusModel$lock()
