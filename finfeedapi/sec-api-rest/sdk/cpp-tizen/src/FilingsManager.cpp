...
static gboolean __FilingsManagerresponseHandler(gpointer data)
{
...
static gpointer __FilingsManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__FilingsManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(FilingsManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (FilingsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), v1FilingsGetProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __FilingsManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool FilingsManager::v1FilingsGetAsync(char * accessToken,
	long long cik, std::string ticker, std::string formType, std::string fillingDateStart, std::string fillingDateEnd, std::string reportDateStart, std::string reportDateEnd, std::string itemsContain, int pageSize, int pageNumber, DTO.FilingSortBy sortBy, std::string sortOrder, 
	void(* handler)(std::list<DTO.FilingMetadataDto>, Error, void* )
	, void* userData)
{
...
bool FilingsManager::v1FilingsGetSync(char * accessToken,
	long long cik, std::string ticker, std::string formType, std::string fillingDateStart, std::string fillingDateEnd, std::string reportDateStart, std::string reportDateEnd, std::string itemsContain, int pageSize, int pageNumber, DTO.FilingSortBy sortBy, std::string sortOrder, 
	void(* handler)(std::list<DTO.FilingMetadataDto>, Error, void* )
	, void* userData)
{
