...
static gboolean __DownloadManagerresponseHandler(gpointer data)
{
...
static gpointer __DownloadManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__DownloadManagerresponseHandler, static_cast<gpointer>(request));
	...
	void(* handler)(Error, void* ) , void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(DownloadManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (DownloadManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), v1DownloadGetProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __DownloadManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool DownloadManager::v1DownloadGetAsync(char * accessToken,
	std::string accessionNo, std::string fileName, 
...
bool DownloadManager::v1DownloadGetSync(char * accessToken,
	std::string accessionNo, std::string fileName, 
