...
FullTextManager::FullTextManager()
{
...
static gboolean __FullTextManagerresponseHandler(gpointer data)
{
...
static gpointer __FullTextManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__FullTextManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(FullTextManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (FullTextManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), v1FullTextGetProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __FullTextManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool FullTextManager::v1FullTextGetAsync(char * accessToken,
	std::string formType, std::string fillingDateStart, std::string fillingDateEnd, std::string textContains, std::string textNotContain, int pageSize, int pageNumber, std::string sortBy, std::string sortOrder, 
	void(* handler)(std::list<DTO.SecFilingResultDto>, Error, void* )
	, void* userData)
{
...
bool FullTextManager::v1FullTextGetSync(char * accessToken,
	std::string formType, std::string fillingDateStart, std::string fillingDateEnd, std::string textContains, std::string textNotContain, int pageSize, int pageNumber, std::string sortBy, std::string sortOrder, 
	void(* handler)(std::list<DTO.SecFilingResultDto>, Error, void* )
	, void* userData)
{
