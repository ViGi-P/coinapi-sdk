...
#include "XbrlConverterManager.h"
#include "NetClient.h"
#include "Helpers.h"
#include "Error.h"
...
XbrlConverterManager::XbrlConverterManager()
{
...
static gboolean __XbrlConverterManagerresponseHandler(gpointer data)
{
...
static gpointer __XbrlConverterManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__XbrlConverterManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(XbrlConverterManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (XbrlConverterManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), v1XbrlConverterGetProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __XbrlConverterManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool XbrlConverterManager::v1XbrlConverterGetAsync(char * accessToken,
	std::string htmUrl, std::string xbrlUrl, std::string accessionNo, 
	void(* handler)(std::map<string,string>, Error, void* )
	, void* userData)
{
...
bool XbrlConverterManager::v1XbrlConverterGetSync(char * accessToken,
	std::string htmUrl, std::string xbrlUrl, std::string accessionNo, 
	void(* handler)(std::map<string,string>, Error, void* )
	, void* userData)
{
