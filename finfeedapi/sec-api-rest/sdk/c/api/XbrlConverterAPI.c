...
//
list_t*_t*
XbrlConverterAPI_v1XbrlConverterGet(apiClient_t *apiClient, char *htm_url, char *xbrl_url, char *accession_no)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *XbrlConverterAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(XbrlConverterAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, XbrlConverterAPIlocalVarJSON)
        {
        ...
        cJSON_Delete( XbrlConverterAPIlocalVarJSON);
