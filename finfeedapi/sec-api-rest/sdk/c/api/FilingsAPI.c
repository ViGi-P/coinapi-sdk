...
#include <stdio.h>
#include <ctype.h>
#include "FilingsAPI.h"

#define MAX_NUMBER_LENGTH 16
#define MAX_BUFFER_LENGTH 4096

// Functions for enum  for FilingsAPI_v1FilingsGet
...
FilingsAPI_v1FilingsGet(apiClient_t *apiClient, long cik, char *ticker, char *form_type, char *filling_date_start, char *filling_date_end, char *report_date_start, char *report_date_end, char *items_contain, int *page_size, int *page_number, dto_filing_sort_by_e sort_by, char *sort_order)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *FilingsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(FilingsAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, FilingsAPIlocalVarJSON)
        {
        ...
        cJSON_Delete( FilingsAPIlocalVarJSON);
