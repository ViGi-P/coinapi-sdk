...
// Functions for enum  for ExtractorAPI_v1ExtractorGet
...
// Functions for enum  for ExtractorAPI_v1ExtractorItemGet
...
ExtractorAPI_v1ExtractorGet(apiClient_t *apiClient, char *accession_number, dto_extractor_type_e type)
{
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *ExtractorAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(ExtractorAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, ExtractorAPIlocalVarJSON)
        {
        ...
        cJSON_Delete( ExtractorAPIlocalVarJSON);
