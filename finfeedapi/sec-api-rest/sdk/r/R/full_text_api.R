...
#'
#' @docType class
#' @title FullText operations
#' @description FullTextApi
...
#' @export
FullTextApi <- R6::R6Class(
  "FullTextApi",
  public = list(
    api_client = NULL,

    #' @description
    #' Initialize a new FullTextApi.
    #'
    ...
    V1FullTextGetWithHttpInfo = function(form_type = NULL, filling_date_start = NULL, filling_date_end = NULL, text_contains = NULL, text_not_contain = NULL, page_size = NULL, page_number = NULL, sort_by = "AccessionNumber", sort_order = "asc", data_file = NULL, ...) {
      ...
      if (!str_detect(`filling_date_start`, "^\\d{4}-\\d{2}-\\d{2}$")) {
        stop("Invalid value for `filling_date_start` when calling FullTextApi$V1FullTextGet, must conform to the pattern ^\\d{4}-\\d{2}-\\d{2}$.")
      }

      if (!str_detect(`filling_date_end`, "^\\d{4}-\\d{2}-\\d{2}$")) {
        stop("Invalid value for `filling_date_end` when calling FullTextApi$V1FullTextGet, must conform to the pattern ^\\d{4}-\\d{2}-\\d{2}$.")
      }
      ...
      if (!str_detect(`sort_order`, "^(asc|desc)$")) {
        stop("Invalid value for `sort_order` when calling FullTextApi$V1FullTextGet, must conform to the pattern ^(asc|desc)$.")
      }
