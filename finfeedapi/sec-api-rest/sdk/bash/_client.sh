...
case $state in
  ops)
    # Operations
    _values "Operations" \
            "v1DownloadGet[Download file from SEC EDGAR archive]"             "v1ExtractorGet[Extract and classify SEC filing content]" \
...
"sort_order=:[QUERY] Sort order (asc or desc, default: desc)"
      ...
      v1FullTextGet)
        local -a _op_arguments
        _op_arguments=(
                    "form_type=:[QUERY] Filter by form type (e.g., \&quot;10-K\&quot;, \&quot;8-K\&quot;). Multiple values can be comma-separated"
"filling_date_start=:[QUERY] Filter by filling date start (inclusive), format YYYY-MM-DD"
"filling_date_end=:[QUERY] Filter by filling date end (inclusive), format YYYY-MM-DD"
