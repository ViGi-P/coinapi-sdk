...
func main() {
	...
	resp, r, err := apiClient.FullTextAPI.V1FullTextGet(context.Background()).FormType(formType).FillingDateStart(fillingDateStart).FillingDateEnd(fillingDateEnd).TextContains(textContains).TextNotContain(textNotContain).PageSize(pageSize).PageNumber(pageNumber).SortBy(sortBy).SortOrder(sortOrder).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `FullTextAPI.V1FullTextGet``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `V1FullTextGet`: []DTOSecFilingResultDto
	fmt.Fprintf(os.Stdout, "Response from `FullTextAPI.V1FullTextGet`: %v\n", resp)
}
```
