...
func main() {
	...
	resp, r, err := apiClient.XbrlConverterAPI.V1XbrlConverterGet(context.Background()).HtmUrl(htmUrl).XbrlUrl(xbrlUrl).AccessionNo(accessionNo).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `XbrlConverterAPI.V1XbrlConverterGet``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `V1XbrlConverterGet`: map[string]interface{}
	fmt.Fprintf(os.Stdout, "Response from `XbrlConverterAPI.V1XbrlConverterGet`: %v\n", resp)
}
```
