...
class QuotesApi(baseUrl: String) {
   ...
   */
  def v1QuotesSymbolIdCurrentGet(symbolId: String)(implicit apiKey: ApiKeyValue, bearerToken: BearerToken): ApiRequest[QuoteTrade] =
    ApiRequest[QuoteTrade](ApiMethods.GET, baseUrl, "/v1/quotes/{symbol_id}/current", "application/json")
      .withApiKey(apiKey, "Authorization", HEADER)
      .withCredentials(bearerToken).withPathParam("symbol_id", symbolId)
   ...
   * @param limit Amount of items to return (optional, mininum is 1, maximum is 100000, default value is 100, if the parameter is used then every 100 output items are counted as one request)
   */
  def v1QuotesSymbolIdLatestGet(symbolId: String, limit: Option[Int] = None)(implicit apiKey: ApiKeyValue, bearerToken: BearerToken): ApiRequest[Seq[Quote]] =
