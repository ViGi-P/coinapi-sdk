...
module OpenapiClient
  ...
  class OptionsOptionExchangeGroup < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::OptionsOptionExchangeGroup` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::OptionsOptionExchangeGroup`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'time_expiration')
        self.time_expiration = attributes[:'time_expiration']
      end

      if attributes.key?(:'strikes')
        if (value = attributes[:'strikes']).is_a?(Array)
          self.strikes = value
        end
      end
    end
    ...
    def hash
      [asset_id_base, asset_id_quote, underlying_price, time_expiration, strikes].hash
    end
