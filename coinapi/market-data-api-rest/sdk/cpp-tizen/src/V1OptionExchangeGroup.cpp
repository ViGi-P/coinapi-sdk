...
V1.OptionExchangeGroup::V1.OptionExchangeGroup()
{
	//__init();
}
...
void
V1.OptionExchangeGroup::__cleanup()
{
...
void
V1.OptionExchangeGroup::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<V1.Strike> new_list;
			V1.Strike inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("V1.Strike")) {
					jsonToValue(&inst, temp_json, "V1.Strike", "");
				} else {
...
V1.OptionExchangeGroup::V1.OptionExchangeGroup(char* json)
{
	this->fromJson(json);
}

char*
V1.OptionExchangeGroup::toJson()
{
	...
	if (isprimitive("V1.Strike")) {
		list<V1.Strike> new_list = static_cast<list <V1.Strike> > (getStrikes());
		node = converttoJson(&new_list, "V1.Strike", "array");
	} else {
		...
		for (list<V1.Strike>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			V1.Strike obj = *it;
...
std::string
V1.OptionExchangeGroup::getAssetIdBase()
{
	return asset_id_base;
}

void
V1.OptionExchangeGroup::setAssetIdBase(std::string  asset_id_base)
{
	this->asset_id_base = asset_id_base;
}

std::string
V1.OptionExchangeGroup::getAssetIdQuote()
{
	return asset_id_quote;
}

void
V1.OptionExchangeGroup::setAssetIdQuote(std::string  asset_id_quote)
{
	this->asset_id_quote = asset_id_quote;
}

double
V1.OptionExchangeGroup::getUnderlyingPrice()
{
	return underlying_price;
}

void
V1.OptionExchangeGroup::setUnderlyingPrice(double  underlying_price)
{
	this->underlying_price = underlying_price;
}

std::string
V1.OptionExchangeGroup::getTimeExpiration()
{
	return time_expiration;
}

void
V1.OptionExchangeGroup::setTimeExpiration(std::string  time_expiration)
{
	this->time_expiration = time_expiration;
}

std::list<V1.Strike>
V1.OptionExchangeGroup::getStrikes()
{
	return strikes;
}

void
V1.OptionExchangeGroup::setStrikes(std::list <V1.Strike> strikes)
{
