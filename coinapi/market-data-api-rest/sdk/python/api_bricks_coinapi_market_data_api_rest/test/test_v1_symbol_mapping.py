# coding: utf-8

"""
    CoinAPI Market Data REST API

    Cryptocurrency market data: metadata, quotes, books, trades, OHLCV, rates, metrics, options.

    The version of the OpenAPI document: v1
    Contact: support@apibricks.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from api_bricks_coinapi_market_data_api_rest.models.v1_symbol_mapping import V1SymbolMapping

class TestV1SymbolMapping(unittest.TestCase):
    """V1SymbolMapping unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1SymbolMapping:
        """Test V1SymbolMapping
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1SymbolMapping`
        """
        model = V1SymbolMapping()
        if include_optional:
            return V1SymbolMapping(
                symbol_id = '',
                symbol_id_exchange = '',
                coinapi_datainfo_id = 56,
                asset_id_base_exchange = '',
                asset_id_quote_exchange = '',
                asset_id_base = '',
                asset_id_quote = '',
                price_precision = 1.337,
                size_precision = 1.337
            )
        else:
            return V1SymbolMapping(
        )
        """

    def testV1SymbolMapping(self):
        """Test V1SymbolMapping"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
