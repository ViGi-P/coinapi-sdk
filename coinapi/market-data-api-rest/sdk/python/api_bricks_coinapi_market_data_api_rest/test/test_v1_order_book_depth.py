# coding: utf-8

"""
    CoinAPI Market Data REST API

    Cryptocurrency market data: metadata, quotes, books, trades, OHLCV, rates, metrics, options.

    The version of the OpenAPI document: v1
    Contact: support@apibricks.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from api_bricks_coinapi_market_data_api_rest.models.v1_order_book_depth import V1OrderBookDepth

class TestV1OrderBookDepth(unittest.TestCase):
    """V1OrderBookDepth unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1OrderBookDepth:
        """Test V1OrderBookDepth
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1OrderBookDepth`
        """
        model = V1OrderBookDepth()
        if include_optional:
            return V1OrderBookDepth(
                symbol_id = '',
                time_exchange = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                time_coinapi = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                ask_levels = 56,
                bid_levels = 56,
                ask_depth = 1.337,
                bid_depth = 1.337
            )
        else:
            return V1OrderBookDepth(
        )
        """

    def testV1OrderBookDepth(self):
        """Test V1OrderBookDepth"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
