# coding: utf-8

"""
    CoinAPI Market Data REST API

    Cryptocurrency market data: metadata, quotes, books, trades, OHLCV, rates, metrics, options.

    The version of the OpenAPI document: v1
    Contact: support@apibricks.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from api_bricks_coinapi_market_data_api_rest.models.v1_exchange_rates_timeseries_item import V1ExchangeRatesTimeseriesItem

class TestV1ExchangeRatesTimeseriesItem(unittest.TestCase):
    """V1ExchangeRatesTimeseriesItem unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1ExchangeRatesTimeseriesItem:
        """Test V1ExchangeRatesTimeseriesItem
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1ExchangeRatesTimeseriesItem`
        """
        model = V1ExchangeRatesTimeseriesItem()
        if include_optional:
            return V1ExchangeRatesTimeseriesItem(
                time_period_start = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                time_period_end = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                time_open = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                time_close = datetime.datetime.strptime('2013-10-20 19:20:30.00', '%Y-%m-%d %H:%M:%S.%f'),
                rate_open = 1.337,
                rate_high = 1.337,
                rate_low = 1.337,
                rate_close = 1.337
            )
        else:
            return V1ExchangeRatesTimeseriesItem(
        )
        """

    def testV1ExchangeRatesTimeseriesItem(self):
        """Test V1ExchangeRatesTimeseriesItem"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
