...
V1QuoteTrade<PSCustomObject>
#>

function Initialize-V1QuoteTrade {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => V1QuoteTrade' | Write-Debug
...
Convert from JSON to V1QuoteTrade<PSCustomObject>
...
V1QuoteTrade<PSCustomObject>
#>
function ConvertFrom-JsonToV1QuoteTrade {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => V1QuoteTrade' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in V1QuoteTrade
        $AllProperties = ("symbol_id", "time_exchange", "time_coinapi", "ask_price", "ask_size", "bid_price", "bid_size", "last_trade")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "last_trade"))) { #optional property not found
            $LastTrade = $null
        } else {
            $LastTrade = $JsonParameters.PSobject.Properties["last_trade"].value
        }
