...
main =
  hspec $ modifyMaxSize (const 10) $ do
    describe "JSON instances" $ do
      pure ()
      propMimeEq MimeJSON (Proxy :: Proxy MarketDataMetadataAsset)
      propMimeEq MimeJSON (Proxy :: Proxy MarketDataMetadataChain)
      propMimeEq MimeJSON (Proxy :: Proxy MarketDataMetadataExchange)
      propMimeEq MimeJSON (Proxy :: Proxy MarketDataMetadataIcon)
      propMimeEq MimeJSON (Proxy :: Proxy MarketDataMetadataSymbol)
      ...
      propMimeEq MimeJSON (Proxy :: Proxy V1OptionExchangeGroup)
      ...
      propMimeEq MimeJSON (Proxy :: Proxy V1Strike)
