...
module ExchangeRatesHistoricalREST.Client where
...
dispatchLbs
  ...
  -> ExchangeRatesHistoricalRESTConfig -- ^ config
  -> ExchangeRatesHistoricalRESTRequest req contentType res accept -- ^ request
...
runConfigLog
  :: P.MonadIO m
  => ExchangeRatesHistoricalRESTConfig -> LogExec m a
runConfigLog config = configLogExecWithContext config (configLogContext config)

-- | Run a block using the configured logger instance (logs exceptions)
runConfigLogWithExceptions
  :: (E.MonadCatch m, P.MonadIO m)
  => T.Text -> ExchangeRatesHistoricalRESTConfig -> LogExec m a
