...
PeriodsAPI_v1MetadataPeriodsGet(apiClient_t *apiClient)
{
    ...
    // create the path
    char *localVarPath = strdup("/v1/metadata/periods");
    ...
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *PeriodsAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        if(!cJSON_IsArray(PeriodsAPIlocalVarJSON)) {
            return 0;//nonprimitive container
        }
        elementToReturn = list_createList();
        cJSON *VarJSON;
        cJSON_ArrayForEach(VarJSON, PeriodsAPIlocalVarJSON)
        {
        ...
        cJSON_Delete( PeriodsAPIlocalVarJSON);
