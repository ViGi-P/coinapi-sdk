...
static bool v1MetadataPeriodsGetProcessor(MemoryStruct_s p_chunk, long code, char* errormsg, void* userData,
	void(* voidHandler)())
{
	void(* handler)(std::list<Models.TimeseriesPeriod>, Error, void* )
	= reinterpret_cast<void(*)(std::list<Models.TimeseriesPeriod>, Error, void* )> (voidHandler);
	
	JsonNode* pJson;
	char * data = p_chunk.memory;

	std::list<Models.TimeseriesPeriod> out;
	...
	if (code >= 200 && code < 300) {
		...
		for(guint i = 0; i < length; i++){
			...
			Models.TimeseriesPeriod singlemodel;
