...
PeriodsApi <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return array[ModelsTimeseriesPeriod]
    V1MetadataPeriodsGet = function(data_file = NULL, ..., .parse = TRUE) {
      local_var_response <- self$V1MetadataPeriodsGetWithHttpInfo(data_file = data_file, ..., .parse = .parse)
    ...
    #' @param .parse Logical. If \code{TRUE} then the response will be parsed to a generated type. If \code{FALSE} the response will be returned as unparsed text.
    #'
    #' @return API response (array[ModelsTimeseriesPeriod]) with additional information such as HTTP status code, headers
    V1MetadataPeriodsGetWithHttpInfo = function(data_file = NULL, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
      ...
      if (local_var_resp$status_code >= 200 && local_var_resp$status_code <= 299) {
        ...
        deserialized_resp_obj <- tryCatch(
          self$api_client$DeserializeResponse(local_var_resp, "array[ModelsTimeseriesPeriod]"),
          error = function(e) {
