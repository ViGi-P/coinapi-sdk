...
class ModelsIndexTimeseriesItem {
  /// Returns a new [ModelsIndexTimeseriesItem] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is ModelsIndexTimeseriesItem &&
  ...
  @override
  String toString() => 'ModelsIndexTimeseriesItem[timePeriodStart=$timePeriodStart, timePeriodEnd=$timePeriodEnd, timeOpen=$timeOpen, timeClose=$timeClose, valueOpen=$valueOpen, valueHigh=$valueHigh, valueLow=$valueLow, valueClose=$valueClose, valueCount=$valueCount]';
  ...
  /// Returns a new [ModelsIndexTimeseriesItem] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static ModelsIndexTimeseriesItem? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "ModelsIndexTimeseriesItem[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "ModelsIndexTimeseriesItem[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<ModelsIndexTimeseriesItem> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <ModelsIndexTimeseriesItem>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = ModelsIndexTimeseriesItem.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, ModelsIndexTimeseriesItem> mapFromJson(dynamic json) {
    final map = <String, ModelsIndexTimeseriesItem>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = ModelsIndexTimeseriesItem.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of ModelsIndexTimeseriesItem-objects as value to a dart map
  static Map<String, List<ModelsIndexTimeseriesItem>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<ModelsIndexTimeseriesItem>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = ModelsIndexTimeseriesItem.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
